...
namespace ts {
  ...
  struct MatrixT {
    ...
    */
    ...
    explicit MatrixT(const math::vec3<ValueType>& v) {
      translate(v);
    }
    ...
    void scale(const math::vec3<ValueType>& v) {
      MatrixT tmp;
      tmp.m[0] = v.x;
      tmp.m[5] = v.y;
      tmp.m[10] = v.z;
    ...
    void translate(const math::vec3<ValueType>& v) {
    ...
    // rotateのヘルパ
    void rotateX(const ValueType& ang) { rotate(ang, math::vec3<ValueType>(1.0f,0.0f,0.0f)); }
    void rotateY(const ValueType& ang) { rotate(ang, math::vec3<ValueType>(0.0f,1.0f,0.0f)); }
    void rotateZ(const ValueType& ang) { rotate(ang, math::vec3<ValueType>(0.0f,0.0f,1.0f)); }
    void rotateXrad(const ValueType& ang) { rotate(ang, math::vec3<ValueType>(1.0f,0.0f,0.0f),Radian); }
    void rotateYrad(const ValueType& ang) { rotate(ang, math::vec3<ValueType>(0.0f,1.0f,0.0f),Radian); }
    void rotateZrad(const ValueType& ang) { rotate(ang, math::vec3<ValueType>(0.0f,0.0f,1.0f),Radian); }

    // rotate glRotateと同じ
    // vector axisを軸にangだけ回転
    void rotate(const ValueType& ang, const math::vec3<ValueType>& axis, AngleType angt=Angle) {
	  MatrixT tmp;
    ...
    math::vec3<ValueType> mult2d(const math::vec3<ValueType>& v) const {
      math::vec3<ValueType> dst;
    ...
    math::vec3<ValueType> mult(const math::vec3<ValueType>& v) const {
      math::vec3<ValueType> dst;
      dst.x = v.x * m[0] + v.y * m[4] + v.z * m[ 8] + m[12];
      dst.y = v.x * m[1] + v.y * m[5] + v.z * m[ 9] + m[13];
      dst.z = v.x * m[2] + v.y * m[6] + v.z * m[10] + m[14];
    ...
    // OpenGLのglLookAtと同じ
    void lookAt(const math::vec3<ValueType>& eye, const math::vec3<ValueType>& center, const math::vec3<ValueType>& up) {
      MatrixT<ValueType> m;
      math::vec3<ValueType> x, y, z;
      ValueType mag;
      ...
      *this *= m;
      ...
      this->translate(math::vec3<ValueType>(-eye.x, -eye.y, -eye.z));
    }
    ...
    bool unProject(ValueType winx, ValueType winy, ValueType winz,
	                 ...
	                 const math::rect_i& viewport,
            	     math::vec3<ValueType>& obj) {
      MatrixT mM;
      MatrixT mA;
      ValueType in[4];
      ValueType out[4];
