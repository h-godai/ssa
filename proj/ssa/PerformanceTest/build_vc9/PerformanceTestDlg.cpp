...
void PerformanceTestDlg::DoDataExchange(CDataExchange* pDX)
{
  ...
  DDX_Text(pDX, IDC_EDIT_RESULT5, m_result5);
  DDX_Text(pDX, IDC_EDIT_RESULT6, m_result6);
...
BEGIN_MESSAGE_MAP(PerformanceTestDlg, CDialog)
    ...
    ON_BN_CLICKED(IDC_BUTTON5, &PerformanceTestDlg::OnBnClickedButton5)
    ON_BN_CLICKED(IDC_BUTTON7, &PerformanceTestDlg::OnBnClickedButton6)
    ON_BN_CLICKED(IDC_BUTTON_ALL, &PerformanceTestDlg::OnBnClickedButtonAll)
    ON_CBN_SELCHANGE(IDC_COMBO_KIND, &PerformanceTestDlg::OnCbnSelchangeComboKind)
    ON_BN_CLICKED(IDC_BUTTON_SAVE, &PerformanceTestDlg::OnBnClickedButtonSave)
END_MESSAGE_MAP()
...
void PerformanceTestDlg::OnBnClickedButton5()
{
  m_performanceTestForm->startTest(m_kindComboBox.GetCurSel(), 5);
}
void PerformanceTestDlg::OnBnClickedButton6()
{
  m_performanceTestForm->startTest(m_kindComboBox.GetCurSel(), 6);
}


void PerformanceTestDlg::OnBnClickedButtonAll()
{
  m_performanceTestForm->startTest(m_kindComboBox.GetCurSel(), 0);
}

void PerformanceTestDlg::OnCbnSelchangeComboKind()
{
  m_result1 = _T("");
  m_result2 = _T("");
  m_result3 = _T("");
  ...
  m_result6 = _T("");
  UpdateData(FALSE);
  m_performanceTestForm->reset();
}

void PerformanceTestDlg::OnBnClickedButtonSave()
{
  // ファイル保存ダイアログ
  ...
  m_performanceTestForm->saveResult(fileName.GetBuffer());
}
