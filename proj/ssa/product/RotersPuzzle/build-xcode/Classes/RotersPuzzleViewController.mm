...
@interface RotersPuzzleViewController ()
...
@implementation RotersPuzzleViewController
...
- (void)awakeFromNib
{
  ...
  app::app_instance().initialize(info);

  app::app_instance().gr_.set_screen_size(((EAGLView *)self.view).framebufferWidth,
										 ((EAGLView *)self.view).framebufferHeight);


  // soundの登録
  std::vector<std::string> soundfn;
  app::app_instance().get_sound_list(soundfn);
  if (!soundfn.empty()) {
  ...
  app::app_instance().get_music_list(musicfn);
  if (!musicfn.empty()) {
...
- (void)audio_hook
{
  ...
  app::app_instance().audio_control_hook(params);
...
{
  [(EAGLView *)self.view setFramebuffer];
  app::app_instance().draw_frame_hook(0.0f);
...
//  UITouch *touch = [[event allTouches] anyObject];
  for(UITouch *touch in [touches allObjects]) {
	...
	app::app_instance().notify_event(tap_ctrl(tap_ctrl::touch, ts::math::pos2_f(pos.x, pos.y)), idx);
  }
  [self audio_hook];
}
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  for(UITouch *touch in [touches allObjects]) {
	...
	if(idx != NSNotFound) {
	  ...
	  app::app_instance().notify_event(tap_ctrl(tap_ctrl::release, ts::math::pos2_f(pos.x, pos.y)), idx);
	}
  }
  [self audio_hook];
}  
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  for(UITouch *touch in [touches allObjects]) {
	NSUInteger idx = [touchArray_ indexOfObject:touch];
	CGPoint pos = [touch locationInView:self.view];
	app::app_instance().notify_event(tap_ctrl(tap_ctrl::move, ts::math::pos2_f(pos.x, pos.y)), idx);
  }
  [self audio_hook];
}  
...
-(void)accelerometer:(UIAccelerometer *)accelerometer didAccelerate:(UIAcceleration *)acceleration{
  using namespace ts::appfw::in;
  app::app_instance().notify_event(acc_ctrl(acceleration.x, acceleration.y, acceleration.z));
}
